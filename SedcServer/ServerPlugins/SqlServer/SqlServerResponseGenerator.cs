    ...
    public class SqlServerResponseGenerator : IResponseGenerator, IDisposable
    {
        ...
        private async Task<Response> GetResponse(SqlResponseCommand command)
        {
            ...
            catch (Exception )
            {
                ...
                var responder = new NoConectivity();
                return await responder.GetResponse();
            }
        ...
        public bool IsInterested(Request request, ILogger logger)
        {
            ...
            var pathTwo = $"sql/{DatabaseName}";

            return (request.Path.StartsWith(path, StringComparison.InvariantCultureIgnoreCase) || ((request.Path.StartsWith(pathTwo, StringComparison.InvariantCultureIgnoreCase)) && request.Path.EndsWith(DatabaseName, StringComparison.InvariantCultureIgnoreCase)));
        }
